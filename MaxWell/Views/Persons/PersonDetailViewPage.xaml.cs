...
namespace MaxWell.Views.Persons
{
	...
	public partial class PersonDetailViewPage : ContentPage
    {

      
        private ContentPage _tbVisits;

        public PersonDetailViewPage()
        {
            InitializeComponent();
        }
        ...
        protected override async void OnAppearing()
        {
            ...
            if (!Convert.ToInt32(vm.Person.VKUserId).Equals(await App.GetInstance().GetVkUserId()))
            {
                ToolbarItems.Clear();
                personDetailView.HideButton();
            }
        }
        public PersonDetailViewPage(Person person)
        {
            InitializeComponent();
